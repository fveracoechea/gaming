 ...
 */
export const teamMember = t.pgTable("team_member", {
  id,
  teamId: t
    .text()
    .notNull()
  ...
  t.uniqueIndex("team_one_captain_per_team_idx").on(table.teamId).where(
    eq(table.role, "CAPTAIN"),
  ),
]);
 ...
 */
export const tournamentTeamParticipant = t.pgTable(
  "tournament_team_participant",
  {
    ...
    status: t
      .varchar()
      .$type<TeamTournamentParticipantStatus>()
      .notNull()
      .default("REGISTERED"),
    placement: t.integer(),
