...
const validator = createValidator(CreateTeamSchema);

export async function action({ context, request, params: { teamId } }: Route.ActionArgs) {
  const rpc = getORPCClient(context);
  const [fieldErrors, formValues] = await validator.tryValidate(await request.json());
...
export function TeamEditForm({ id, team }: Props) {
  ...
  return (
    <form id={id} noValidate method="post" onSubmit={onSubmit}>
      <FieldGroup>
